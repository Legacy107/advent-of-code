let valMap = ['A', 'K', 'Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2', 'J'].reverse()
let res = `input-here`
  .split('\n')
  .map(el => el.split(' '))
  .map(el => [
  ...
  .map(el => [el[0], el[1], (() => {
    ...
    let cnt = 0
    let last = ''
    for (let i = 0; i < arr.length && arr[i] !== 'J'; i++) {
      if (last === arr[i]) {
        cnt++
      } else if (cnt) {
        if (cnt > max) {
    ...
    if (!max_char) return arr
    return arr
      .map(char => char === 'J' ? max_char : char)
      .sort((a, b) => valMap.indexOf(b) - valMap.indexOf(a))
  })()])
  .map(el => [...el, (() => {
    let arr = el[2]
    let val = Array.from({
